...
class Equipamento_Model extends CI_Model
{
    ...
    public function getcondesadora($distribuidor, $produto, $serie, $emissao = NULL)
    {   
        // Adicionar condições para distribuidor, produto e série
        $this->db->where('distribuidor_id', $distribuidor);
    ...
    // Método para verificar se distribuidor existe na tabela equip_condesadora_serie
    public function distribuidorExists($distribuidor_id)
    {
        $this->db->select('distribuidor_id, distribuidor_nome');
        $this->db->where('distribuidor_id', $distribuidor_id);
        $q = $this->db->get('distribuidores');
        
        if ($q->num_rows() > 0) {
            return $q->row(); // Retorna o objeto da linha encontrada
        } else {
    ...
    public function produtoExists($produto)
    {
        $this->db->where('produto', $produto);
        $q = $this->db->get('equip_condesadora_serie');
        return $q->num_rows() > 0;
    ...
    // Método para verificar se série existe
    ...
    // Método para verificar se a série está em uso
    public function isUsed($distribuidor_id, $produto, $serie)
    {
        $this->db->where('distribuidor_id', $distribuidor_id);
        $this->db->where('produto', $produto);
        $this->db->where('serie', $serie);
