...
class Obra_Model extends CI_Model
{
	...
	*/
    ...
    //##################	Distribuidor	###########################################
    public function getObrasDistribuidor() {
        
        // Selecione os campos necessários da tabela obra e o nome do distribuidor
        $this->db->select('obra.*, d.distribuidor_nome as distribuidor_nome');
        
        // Especifique as tabelas a serem unidas e as condições de junção
        $this->db->from('obra');
        $this->db->join('distribuidores d', 'obra.obra_distribuidor = d.distribuidor_id', 'left');
