					...
					<div class="row">
						<div class="col-md-12">
							...
							<table id="dados-empresa" class="table table-striped table-bordered dt-responsive nowrap">
								<tbody>
									...
									<tr>
										<td>CREDENCIADO</td>
										<td><span class="lock"><?= $cliente->cliente_credenciado == 1 ? "Sim" : "Não"; ?></span></td>
									</tr>
									<tr>
						...
						<div class="botoes-aprovacao col-md-12">
							<div class="row">
								...
								<div class="col-md-6">
									<button type="submit" class="btn btn-primary btn-filtro parceiros"> Atualizar <i class="fa fa-angle-double-right" aria-hidden="true"></i></button>
								</div>
							</div>
						</div>
    ...
    function getDistribuidores() {
        ...
        dataType: 'json',
            success:function(data){
	            ...
	            // Atualizar o selectpicker após adicionar as opções
	            $('.selectpicker').selectpicker('refresh');

	            // Pré-selecionar opções específicas obtidas do banco de dados
	            let selDistribuidores = '<?= $distribuidores ?>';
	            let distribuidoresArray = selDistribuidores.split(','); // Separar os IDs por vírgula

	            // Limpar espaços em branco
	            distribuidoresArray = distribuidoresArray.map(function(item) {
	                return item.trim();
	            });

	            // Definir os valores selecionados do selectpicker
	            $('#distribuidores_parceiros').val(distribuidoresArray);

	            // Atualizar novamente o selectpicker após pré-selecionar as opções
	            $('.selectpicker').selectpicker('refresh');
