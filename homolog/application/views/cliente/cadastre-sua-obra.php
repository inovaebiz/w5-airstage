...
<div class="container-fluid">
	<div class="row">
		...
		<main role="main" class="col-md-12 col-lg-9 col-xl-8 offset-xl-1">
			...
			<section>
				<div class="row">
					...
					<form id="needValidation" role="form" action="" method="post" class="f1" enctype="multipart/form-data" onsubmit="return montaKit();" autocomplete="off">
						...
						<input type="hidden" name="campanha_id" id="campanha_id" value="<?=$camp->campanha_id?>">
						...
						<fieldset>
							...
							<div class="row">
								<div class="form-group col-md">
									...
									<input type="text" name="obra_nome" placeholder="Nome da Obra/Cliente *" data-field="Nome da Obra/Cliente" class="validation form-control">
								</div>
								<div class="form-group col-md">
								...
								<div class="form-group col-md">
									...
									<input type="date" min="<?= $camp->campanha_data_inicial ?>" name="obra_data_instalacao" id="obra_data_instalacao" placeholder="Data da Instalação *" class="validation form-control" onblur="validaData()" required>
								</div>
							</div>
							...
							<div class="row">
								...
								<div class="form-group col-md-4">
									...
									<input type="text" class="validation form-control " name="obra_nome_instalador_vendedor_responsavel" placeholder="Nome do Instalador/Vendedor Responsável pela Obra *" required>
								</div>

								<div class="form-group col-md-4">
									<label class="large-label">CPF do Instalador/Vendedor Responsável pela Obra:</label>
									<input type="text" name="obra_cpf_instalador_vendedor_responsavel" placeholder="CPF do Instalador/Vendedor Responsável pela Obra" class="opcional form-control" onblur="validarCpfObra(this.value)" onclick="resetCpfObra()">
								...
								<div class="col-md-12 row-categoria">
									<div class="row">
										<div class="input-group col-md-6">
											<select name="" id="sel" class="form-control row-cat" onchange="cat(this)" required>
									...
									<div class="row">
										...
										<div class="input-group col-md-6">
											<select name="categ[]" id="sel" class="form-control row-tipo item-tipo modelo" onchange="tipo(this)" required>
												<option value="">Condensador *</option>
											</select>
										</div>
										<div class="input-group col-md-6">
											<input type="tel" id="sel" name="eSerie[]" maxlength="10" minlength="6" placeholder="Número de série *" class="form-control row-tipo item-tipo" id="validaSerieCondensadora" onkeyup="validarCondensadora(this)" required>
									...
									<div class="row">
										<div class="input-group col-md-6">
											<select name="equipamento[]" id="sel" class="form-control row-equip">
												<option value="Sem equipamento">Sem equipamento</option>
											</select>
										</div>
										<div class="input-group col-md-6">
											<input type="tel" id="validaSerieEvaporadora" name="nSerie[]" maxlength="7" placeholder="Número de série" class="form-control">
										</div>
									</div>
								</div>
								<!--################################		############################################# -->
			...
			?>
			<div class='alert alert-warning text-center'>Sem campanha ativa.</div>
			<?php
				}
			}
			?>
...
<script>
	...
	function newCat(e) {
		...
		div += '<div class="input-group col-md-6">';
		div += '<select name="" id="" class="form-control row-cat" onchange="cat(this)">';
...
<script>
	...
	*/
		...
		dataType: 'json',
			success:function(data){
				$.each(data, function(key, value){
					$('#obra_distribuidor_data_complete').append('<option value="'+value.id+'">'+value.distribuidor+'</option>');
				});
			}
		});
	}
	...
	let data = {
		// msg_alert_condensadora: "<div class='alert alert-danger'>Distribuidor selecionado errado na tela anterior, Número de série inválido ou já computado em outra obra.</div>",
		// msg_alert_condensadora_in_used: "<div class='alert alert-danger'>Número de série já cadastrado em outra obra.</div>",
		msg_alert_evaporadora: "<div class='alert alert-danger'>Número de série inválido e já computado em obra acima.</div>",
		msg_alert_sucesso: "<div class='alert alert-success'>Distribuidor e Número de série válido.</div>",
	};
	...
	function validarCondensadora(e, clas) {
		...
		let distribuidor = $("#obra_distribuidor_data_complete option:selected").val();
		...
		if (dataSearch.length >= 7) {
			...
			$.ajax({
				...
				error: function(response) {
	                console.error('Erro ao buscar condensadores.');
	            }
			});
